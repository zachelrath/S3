...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        // Get the MPU shadow bucket and check authorization.
        next => {
            const mpuOverviewKey = _getOverviewKey(splitter, objectKey,
                uploadId);
            return metadata.getBucketAndObjectMD(mpuBucketName, mpuOverviewKey,
            log, (err, res) => {
                if (err && err.NoSuchBucket) {
                    return next(errors.NoSuchUpload);
                }
                if (err) {
                    log.error('error getting shadow mpu bucket and object', {
                        error: err,
                        method: 'objectPutPart::metadata.getBucketAndObjectMD',
                    });
                    return next(err);
                }
                const mpuBucket = res.bucket ?
                          BucketInfo.deSerialize(res.bucket) : undefined;
                // BACKWARD: Remove to remove the old splitter
                if (mpuBucket.getMdBucketModelVersion() < 2) {
                    splitter = constants.oldSplitter;
                }
                const obj = res.obj ? JSON.parse(res.obj) : undefined;
                const initiatorID = obj.initiator.ID;
...
<<<<<<< HEAD
=======
        // If there is a pre-existing part, update utapi metrics and get the
...
>>>>>>> 4138e0e... FT: Delete objectPutPart overwritten data
...
<<<<<<< HEAD
                    return next(null, dataGetInfo, hexDigest);
                });
        },
        // Store data locations in metadata and delete any overwritten data.
        (dataGetInfo, hexDigest, next) => {
=======
                    return next(null, dataGetInfo, hexDigest, oldLocations);
                });
        },
        // Store data locations in metadata and delete any overwritten data.
        (dataGetInfo, hexDigest, oldLocations, next) => {
>>>>>>> 4138e0e... FT: Delete objectPutPart overwritten data

...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        next => {
            const mpuOverviewKey = constants.overviewKey(splitter, objectKey,
                uploadId);
        ...
        (dataGetInfo, hexDigest, next) => {
            ...
            const paddedPartNumber = constants.paddedPartNumber(partNumber);
            const partKey = constants.partKey(uploadId, splitter,
                paddedPartNumber);
            const omVal = {

...
function objectGet(authInfo, request, log, callback) {
    ...
    // <versioning>
    //   key scheme: prefix separator objectKey [separator versionId]
    //   where     : prefix is "A" (master) or "V", separator is "|"
    //   example   : "A|foo" for the master version of foo
    //               "V|foo|1234567890" for version 1234567890 of foo
    const objectKey = (request.query.versionId ?
        config.versioning.versionPrefix + config.versioning.separator +
        request.objectKey + config.versioning.separator +
        request.query.versionId : config.versioning.masterPrefix +
        config.versioning.separator + request.objectKey);
    // </versioning>
    const mdValParams = {
        authInfo,
        bucketName,

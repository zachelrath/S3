...
export function listMetrics(metricType) {
    ...
    if (metricType === 'buckets') {
        ...
        commander
            ...
            .option('--users <users>', 'Name of user(s) with a comma ' +
                'separator if more than one');
    }
    commander
    ...
    if (metricType !== 'buckets') {
        ...
        // Only allow 'buckets', 'accounts' or 'users' as the metric
        const validMetrics = ['buckets', 'accounts', 'users'];
        if (validMetrics.indexOf(commander.metric) < 0) {
            logger.error('metric must be \'buckets\', \'accounts\', or ' +
                '\'users\'');

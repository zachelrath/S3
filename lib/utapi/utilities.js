...
function _listMetrics(host,
    ...
    const metricRoutes = {
        ...
        service: '/service?Action=ListMetrics',
    };
    const options = {
        host,
        port,
...
export function listMetrics(metricType) {
    ...
    if (metricType === 'buckets') {
        ...
        commander
            ...
            .option('--service <service>', 'Name of the current service (' +
                `${_config.utapi.component})`);
    }
    commander
    ...
    if (metricType !== 'buckets') {
        ...
        // Only allow 'buckets', 'accounts', or 'service' as the metric
        if (commander.metric !== 'buckets' && commander.metric !== 'accounts'
            && commander.metric !== 'service') {
            logger.error('metric must be buckets, accounts, or service');
    ...
    // Utapi keys are generated based on the value of `_config.utapi.component`.
    // Requesting different service is not possible.
    if (metric === 'service' && commander[metric] !== _config.utapi.component) {
        logger.error('Cannot request service level metrics aside from ' +
            `${_config.utapi.component}`);
        commander.outputHelp();
        process.exit(1);
        return;

...
utils.getNamesFromReq = function getNamesFromReq(request, bucketNameFromHost) {
    ...
    resources.path = path;
    // Slice off leading '/'
    const pathWithoutLeading = path.slice(1);
    let fullHost = undefined;
    const host = headers.host || undefined;
    if (host) {
        index = host.indexOf(':'); 
        fullHost = index !== -1 ? host.substring(0, index) : host;
    }

    if (bucketNameFromHost) {
        resources.bucket = bucketNameFromHost;
        resources.host = fullHost.slice(bucketNameFromHost.length + 1);
        resources.object = pathWithoutLeading;
        resources.gotBucketNameFromHost = true;
    } else {
        resources.host = fullHost;
        index = pathWithoutLeading.indexOf('/') + 1;
        if (index) {
            resources.bucket = pathWithoutLeading.substring(0, index - 1);
            resources.object = pathWithoutLeading.substring(index);
        } else {
            resources.bucket = pathWithoutLeading;
        }
    }
    // remove any empty strings or nulls
    resources.bucket = resources.bucket || undefined;
    resources.object = resources.object || undefined;
    return resources;
};

...
export default function routePUT(request, response, log, statsClient) {
    ...
    const metricObj = { bucket: request.bucketName };
        ...
        request.on('end', () => {
            ...
            if (request.query.acl !== undefined) {
                api.callApiMethod('bucketPutACL', request, log, err => {
                    ...
                    pushMetrics(err, log, 'bucketPutACL', metricObj);
                ...
                if (request.post) {
                    ...
                    return parseString(xmlToParse, (err, result) => {
                        ...
                        err => {
                            statsReport500(err, statsClient);
                            pushMetrics(err, log, 'bucketPut', metricObj);
        ...
        if (request.query.partNumber) {
            if (request.headers['x-amz-copy-source']) {
                ...
                api.callApiMethod('objectPutPart', request, log,
                    (err, calculatedHash) => {
                        ...
                        statsReport500(err, statsClient);
                        pushMetrics(err, log, 'objectPutPart', Object.assign({},
                            metricObj, {
            ...
            request.on('end', () => {
                api.callApiMethod('objectPutACL', request, log, err => {
                    statsReport500(err, statsClient);
                    pushMetrics(err, log, 'objectPutACL', metricObj);
                    return routesUtils.responseNoBody(err, null, response, 200,
                        log);
                });
            });
            ...
            return api.callApiMethod('objectCopy', request, log, (err, xml,
                additionalHeaders, newByteLength, oldByteLength) => {
                statsReport500(err, statsClient);
                pushMetrics(err, log, 'objectCopy', Object.assign({},
                    metricObj, {
                        newByteLength,
                        oldByteLength,
                    }));
            ...
            api.callApiMethod('objectPut', request, log,
                (err, contentMD5, oldByteLength) => {
                    // ETag's hex should always be enclosed in quotes
                    statsReport500(err, statsClient);
                    const resMetaHeaders = {
                        ETag: `"${contentMD5}"`,
                    };
                    pushMetrics(err, log, 'objectPut', Object.assign({},
                        metricObj, {
                            newByteLength: request.parsedContentLength,
                            oldByteLength,
                        }));

...
function okContentHeadersResponse(overrideHeaders, resHeaders,
    response, range, log) {
    const addHeaders = {
        'Content-Type': overrideHeaders['response-content-type'],
        'Content-Language': overrideHeaders['response-content-language'],
        'Expires': overrideHeaders['response-expires'],
        'Cache-Control': overrideHeaders['response-cache-control'],
        'Content-Disposition': overrideHeaders['response-content-disposition'],
        'Content-Encoding': overrideHeaders['response-content-encoding'],
    };
    const keys = Object.keys(resHeaders);
    const len = keys.length;
    for (let i = 0; i < len; ++i) {
        const key = keys[i];
        if (addHeaders[key] === undefined) {
            addHeaders[key] = resHeaders[key];
        }
    }
...
const routesUtils = {
    ...
    setPartRanges(dataLocations, outerRange) {
        ...
        const max = end - begin;
        const len = dataLocations.length;
        let total = 0;
        for (let i = 0; i < len; i++) {
            ...
            const partStart = parseInt(location.start, 10);
            const partSize = parseInt(location.size, 10);

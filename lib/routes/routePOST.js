    ...
    request.on('end', () => {
        if (request.query.uploads !== undefined) {
            ...
            api.callApiMethod('initiateMultipartUpload', request, log,
                (err, result) => {
                    pushMetrics(err, log, 'initiateMultipartUpload', metricObj);
            ...
            api.callApiMethod('completeMultipartUpload', request, log,
                (err, result) => {
                    pushMetrics(err, log, 'completeMultipartUpload', metricObj);
                    return routesUtils.responseXMLBody(err, result, response,
                        log);
                });
            ...
            api.callApiMethod('multiObjectDelete', request, log,
                (err, xml, newByteLength, objectsCount) => {
                    pushMetrics(err, log, 'multiObjectDelete', Object.assign({},
                        metricObj, {
                            newByteLength,
                            objectsCount,
                        }));

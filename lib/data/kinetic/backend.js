...
const backend = {
    ...
    get: function y(key, range, reqUids, callback, socket) {
        ...
        kinetic.streamToPDU(socket, (err, pdu) => {
            ...
            return socket.on('data', chunk => {
                ...
                // console.log('this is the chunkSize :::::');
                // console.log(pdu.getChunkSize());
                // console.log('this is the value.length :::::');
                // console.log(value.length);
                if (value.length === pdu.getChunkSize()){
                    ...
                    console.log('Value stream kinetic');
                    console.log(val);
                    return callback(null, val);
                }
            });
            // return socket.on('end', () => {
            //     console.log('enter in the end ------');
            //     console.log(value);
            // });
        });
    },
    ...
    delete: function z(key, reqUids, callback, socket) {
        ...
        kinetic.streamToPDU(socket, (err, pdu) => {
            ...
            const pdu2 = new kinetic.GetPDU(sequence, key);
            ++sequence;

            socket.write(pdu2.read());
            kinetic.streamToPDU(socket, (err, pdu) => {
                console.log('Error from STREAMTOPDU');
                console.log(err);
                console.log('RESPONSE OF GET IN DELETE')
                console.log(kinetic.getErrorName(pdu.getStatusCode()));
                // console.log(util.inspect(
                //     pdu, { showHidden: false, depth: null }));

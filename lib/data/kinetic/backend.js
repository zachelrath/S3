...
const backend = {
    ...
    get: function getKinetic(key, range, reqUids, callback, socket) {
        ...
        if (keyValue.slice(0, 4).toString() !== 'part') {
            ...
            getKinetic(socket, keyValue, log, (err, value) => {
                console.log(value);
                const keyTab = splitBuffer(value, value.length, 20, log);


                // async.eachSeries(
                //     , (key, next) => {
                //         const pdu5 = new kinetic.GetPDU(sequence, key);
                //         ++sequence;

                //         socket.write(pdu5.read());
                //         kinetic.streamToPDU(socket, (err, pdu) => {
                //             if (err) {
                //                 return callback(err);
                //             }
                //             if (pdu.getStatusCode() !== kinetic.errors.SUCCESS) {
                //                 return callback(
                //                     kinetic.getErrorName(pdu5.getStatusCode()));
                //             }

                //             socket.resume();
                //             return socket.on('data', chunk => {
                //                 value = Buffer.concat([value, chunk]);
                //                 if(value.length === pdu5.getChunkSize()) {
                //                     next(null, 'test');
                //                 }
                //             });
                //         });

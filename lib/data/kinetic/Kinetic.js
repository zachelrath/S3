...
class Kinetic {
    ...
    getSocketIndex(drive) {
        for (let i = 0; i < this._sockets.length; i++) {
            console.log('---this._sockets[i]._request');
            console.log(this._sockets[i]._request);
            console.log('---this._sockets[i]._requestGet');
            console.log(this._sockets[i]._requestGet);
            console.log('---this._sockets[i].drive');
            console.log(this._sockets[i].drive);
    ...
    _bindReadable(socket, index) {
        socket.sock.on('readable', err => {
            if (socket._requestGet === false) {
                ...
                this._startGet(index, this._sockets[index]._pdu);
            }
        });
    }
    ...
    _startGet(index, pdu) {
        ...
        const chunk = this._sockets[index].sock.read(
            this._sockets[index]._chunkSize);
        if (chunk !== null){
    ...
    put(index, value, options, callback) {
        ...
        console.log('index----------------');
        console.log(index);
        console.log(this._sockets.length);

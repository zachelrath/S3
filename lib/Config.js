...
class Config {
    ...
    _getConfig() {
        ...
        if (config.serviceCertFilePaths) {
            assert(typeof config.serviceCertFilePaths === 'object' &&
                typeof config.serviceCertFilePaths.key === 'string' &&
                typeof config.serviceCertFilePaths.cert === 'string' && ((
                    config.serviceCertFilePaths.ca &&
                    typeof config.serviceCertFilePaths.ca === 'string') ||
                    !config.serviceCertFilePaths.ca)
               );
        }
        const { key: serviceKey, cert: serviceCert, ca: serviceCa } =
            config.serviceCertFilePaths ? config.serviceCertFilePaths : {};
        if (serviceKey && serviceCert) {
            const keypath = (serviceKey[0] === '/') ?
                serviceKey : `${this._basePath}/${serviceKey}`;
            const certpath = (serviceCert[0] === '/') ?
                serviceCert : `${this._basePath}/${serviceCert}`;
            let capath = undefined;
            if (serviceCa) {
                capath = (serviceCa[0] === '/') ?
                    serviceCa : `${this._basePath}/${serviceCa}`;
                assert.doesNotThrow(() =>
                    fs.accessSync(capath, fs.F_OK | fs.R_OK),
                    `File not found or unreachable: ${capath}`);
            }
            ...
            this.serviceHttps = {
                cert: fs.readFileSync(certpath, 'ascii'),
                key: fs.readFileSync(keypath, 'ascii'),
                ca: serviceCa ? fs.readFileSync(capath, 'ascii') : undefined,
            };
            this.serviceHttpsPath = {
                ca: capath,
                cert: certpath,
            };
        } else if (serviceKey || serviceCert) {
            throw new Error('bad config: both serviceCertFilePaths.key and ' +
                'serviceCertFilePaths.cert must be defined');
        }

...
class BucketFileInterface {
    ...
    metastoreInit() {
        //console.log('COCO',this.cache);
        this.metastore.createReadStream()
          .on('data', (data) => {
    ...
    metastoreGet(key,cb) {
        try {
            const value = this.cache.get(key, true );
            console.log('ALREADY IN CACHE',value);
            if (value == undefined) {
                console.log('BIG MESS');
            }
            //console.log('MY CACHE =>', this.cache);
            return cb(null,value);
        } catch(err) {
            console.log('n2berr',err);
            if (err.errorcode == 'ENOTFOUND'){
                // if not cached Check inside the metastore
                this.metastore.get(key, (err,value) => {
                    if (err){
                        console.log('ERR || FIRST TIME');
                        return cb(err,null);
                    }
                    return cb(null, value);
                });
    ...
    getBucketAttributesNoRef(bucketName, log, cb) {
        ...
        // });
        this.metastoreGet(bucketName, (err,data) => {
            if (err) {
                if (err.notFound) {
                    console.log('SHOULD GO HERE WHEN FIRST TIME')
                    return cb(errors.NoSuchBucket);
                }
                return cb(errors.InternalError, null);
            }
    ...
    putBucketAttributesNoRef(bucketName, bucketMD, log, cb) {
        this.metastore.put(bucketName, bucketMD.serialize(),
                            ...
                            err => {
                                ...
                                // caching
                                this.metastorePut(bucketName, bucketMD.serialize(), (err) => {
                                    if (err){
                                        return cb(err)
                                    }
                                    return cb();
    ...
    deleteBucket(bucketName, log, cb) {
        ...
        this.metastore.del(bucketName,
                           err => {
                               ...
                               this.metastoreDel(bucketName, (err) => {
                                    if (err){
                                        return cb(err);
                                    }

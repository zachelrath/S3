...
class BucketFileInterface {
    ...
    putObject(bucketName, objName, objVal, log, cb) {
        this.loadDBIfExists(bucketName, log, (err, db) => {
            ...
            // <versioning>
            // assign version prefix, assign versionId
            const timestamp = (`0000000000000000${Date.now()}`).substr(-16);
            const vObjName = config.versioning.versionPrefix +
                config.versioning.separator + objName +
                config.versioning.separator + timestamp;
            const mObjName = config.versioning.masterPrefix +
                config.versioning.separator + objName;
            // TODO comparing versions to get the latest: this will require
            // version vector, locking, batching, caching, and a key scheme
            // </versioning>
            const ops = [
                {type: 'put', key: mObjName, value: JSON.stringify(objVal)},
                {type: 'put', key: vObjName, value: JSON.stringify(objVal)}
            ];
            db.batch(ops, OPTIONS, err => {
                this.unRef();
                if (err) {

    ...
    putObjectMD: (bucketName, objName, objVal, log, cb) => {
        ...
        client.putObject(bucketName, objName, objVal, log, err => {
            ...
            for (var key in tagkey) {
                client.putObject(bucketName, tagkey[key], objName, log, err => {
                    if (err) {
                        log.warn('error from metadata', { implName, error: err });
                        return cb(err);
                    }
                    ...
                    log, (err, data) => {
                        if (data) {
                            for (var prefix in data.CommonPrefixes) {
                                if (data.CommonPrefixes[prefix].indexOf("T|") > -1) {
                                    data.CommonPrefixes.splice(prefix,1);
                                }
                            }
                        }
                        if (err) {

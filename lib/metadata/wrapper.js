    ...
    listObject: (bucketName, prefix, marker, delimiter, maxKeys, log, cb) => {
        ...
        client.listObject(bucketName, { prefix, marker, maxKeys, delimiter },
            log, (err, data) => {
                ...
                if (data.Prefix && data.Prefix.indexOf("T|") > -1) {
                    ...
                    if (data) {
                        data.CommonPrefixes.forEach(prefix => {
                            if (prefix.indexOf("T|") > -1)
                                data.CommonPrefixes.splice(data.CommonPrefixes.indexOf(prefix),1);
                        });
                        data.Contents.forEach(elem => {
                            if (elem.key.indexOf("T|") > -1)
                                data.Contents.splice(data.Contents.indexOf(elem),1);
                        });                        
                    }
                ...
                log, (err, data) => {
                    ...
                    data.Contents.forEach(elem => {
                        if (keys.indexOf(elem.key) == -1)
                            delete data.Contents[data.Contents.indexOf(elem)];
                    });
                    return cb(err, data);
                });
            }

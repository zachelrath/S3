...
function writeResponse(res, error, stats, log) {
    const statusCode = error ? error.code : 200;
    const body = Buffer.from(error ?
        JSON.stringify(error) : JSON.stringify(stats), 'utf8');
...
function healthcheckRouteHandler(req, res, log) {
    if (isHealthy()) {
        if (req.method === 'GET' || req.method === 'POST') {
            // get stats
            statsClient.getStats(log, (err, stats) =>
                writeResponse(res, err, stats, log));
        } else {
            writeResponse(res, errors.BadRequest, null, log);
        }
    } else {
        writeResponse(res, errors.InternalError, null, log);
    }
